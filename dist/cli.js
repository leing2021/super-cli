...
export function parse(rawArgs) {
    if (rawArgs.length === 0 || rawArgs.includes("--help") || rawArgs.includes("-h")) {
        return { command: "help", args: [] };
    }
    const builtins = ["list", "doctor", "make"];
    if (builtins.includes(rawArgs[0])) {
        return { command: rawArgs[0], args: rawArgs.slice(1) };
    }
    return { command: "target", target: rawArgs[0], args: rawArgs.slice(1) };
}
function getOpencliTargets() {
    try {
...
function getCliAnythingRegistry() {
    try {
...
function isJsonMode(args) {
    return (args.includes("-f") && args.includes("json") ||
...
function cmdTarget(target, args) {
    ...
    try {
        ...
        if (e.code === "ENOENT" || e.status === 127) {
            ...
            if (plan.backend === "cli-anything") {
                const entry = getCliAnythingRegistry().find((r) => r.name === target);
